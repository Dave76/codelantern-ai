...
This document provides comprehensive configuration guidance for the GitHub MCP Server, including:
- **Working configurations** for GitHub.com Coding Agents (validated)
- **Reference documentation** for all available toolsets
- **Troubleshooting** and best practices

---

## Quick Start: GitHub.com Coding Agent Configuration
...
**Observed result:** `MCP server started successfully (...) with 58 tools` (see verbose logs for the exact list).
...
**Observed result:** `MCP server started successfully (...) with 36 tools` (see verbose logs for the exact list).
...
## Remote GitHub MCP Server Reference
...
The remote server has [additional tools](#additional-remote-server-toolsets) that are not available in the local MCP server, such as the `create_pull_request_with_copilot` tool for invoking Copilot coding agent.
